...
So is this libray, which extends the emacs [[https://www.gnu.org/software/emacs/manual/html_node/elisp/How-Programs-Do-Loading.html#How-Programs-Do-Loading][load]] mechanism so emacs can load org files as lisp source files directly.
...
some for demo, and some for test, so an [[https://orgmode.org/manual/Structure-of-code-blocks.html][org code block]] [[https://orgmode.org/manual/Code-block-specific-header-arguments.html#Code-block-specific-header-arguments][header argument]] ~tangle~ to decide to
...
So when such error occur, we have to handle it by ourself(test is here:ref:test-empty-code-block).
...
If it is not processed correctly, the reader may enter into an infinte loop, especially when using a custom reader to tangle codes.
#+BEGIN_SRC elisp
...
(ert-deftest literate-elisp-read-header-arguments ()
  ...
  (should (equal (literate-elisp-read-header-arguments " :tangle yes") '(:tangle yes)))
  (should (equal (literate-elisp-read-header-arguments " :tangle no  ") '(:tangle no)))
  (should (equal (literate-elisp-read-header-arguments ":tangle yes") '(:tangle yes))))
#+END_SRC
