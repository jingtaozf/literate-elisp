...
end of current code block, so it occur, then the reader will switch back to org mode syntax.
...
a dynamic Boolean variable bound by our read function while parsing is in progress.
It'll indicate whether org mode syntax or elisp mode syntax is in use (which is in an org code block).
#+BEGIN_SRC elisp
...
some for demo, and some for test, so an org code block option ~tangle~ to decide to
