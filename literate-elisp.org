...
The fallback to the org property =literate-load= reads it from
whatever buffer is current, which only means the file being loaded
when the user runs ~literate-elisp-load~ from that file's own buffer.
Called from anywhere else — a hook, another package, an editor RPC —
it interrogates an unrelated buffer, so the guard below asks the
property only while an org buffer is current and otherwise returns
nil, which ~literate-elisp-load-p~ already treats as "load".

The guard is not merely tidiness.  Under Emacs 31 with Org 9.8,
~org-entry-get~ in, say, a Markdown buffer runs Org's headline parser
against that buffer's own ~outline-regexp~, derives a headline of
level 0, and ~org-headline-re~ signals =rx ‘**’ range error= — every
literate load in that session fails until the current buffer happens
to change.
#+BEGIN_SRC elisp
...
** lexical-binding cookie support for .org files
By default Emacs determines a loaded file's =lexical-binding= by reading the first line via
=lisp_file_lexical_cookie= in =src/lread.c=.  That function only recognises =;; -*- ... -*-=
(Lisp comment) or =#! ... ; -*- ... -*-= (shebang) first lines.  An org file's first line is
usually =# -*- mode: org -*-= — starts with =#= followed by a space (not =#!=) — so the cookie
returns =Cookie_None= and Emacs falls back to the global default of =lexical-binding= (typically
=nil=).  The result: every src block in an org file evaluates with *dynamic* binding,
which breaks closures over =let= variables in =run-at-time= / =set-process-filter= /
=set-process-sentinel= lambdas — they signal =void-variable= when fired later.

On Emacs master (post-30.x; first release will be 31), =get_lexical_binding= falls through to the Lisp variable
=internal--get-default-lexical-binding-function= when no cookie is found.  We hook that
function: for =.org= / =.org.elc= files we re-parse the first line for a =lexical-binding=
directive; for everything else we chain to the original handler.

Save the original hook so we can chain through.  This must run at load time, not lazily,
or a reload of literate-elisp would overwrite the saved value with our own replacement.
#+BEGIN_SRC elisp
...
Parse the first line of a file for the cookie.  Returns =:lex= / =:dyn= / =:none=
(=:none= = "no cookie present, defer to caller's default").
#+BEGIN_SRC elisp
...
Helper to chain to the original handler.
#+BEGIN_SRC elisp
...
The hook itself.  =get_lexical_binding= in =src/lread.c= calls us with =FROM=
which can be a string (file path, from direct =Fload=) or a buffer (the
temporary =*load*= buffer, from =Feval_buffer=).  For the buffer case
=buffer-file-name= is usually =nil= during load — we fall back to
=load-file-name= which =Fload= specbinds to the real path while loading.
#+BEGIN_SRC elisp
...
Install the hook.  The =boundp= check makes this a no-op on any Emacs without the variable (every released Emacs as of 30.2; arrives in 31); the =not eq=
check makes it idempotent on reload (so we don't accidentally save our own replacement
as the "original" the second time around).
#+BEGIN_SRC elisp
...
After this hook is installed, an org file whose first line includes
=lexical-binding: t= (e.g. =# -*- mode: org; lexical-binding: t; -*-=) loads
with lexical binding; one without the cookie keeps the previous dynamic-binding
default — backward compatible.

Regression tests live at:
- =tests/test-lexical-binding.org= (three repros that fail without the fix)
- =tests/test-no-cookie.org= (no-cookie org stays dynamic)
...
*** test loading while a foreign buffer is current
A load started from somewhere other than the org file's own buffer —
a hook, another package, an editor RPC — leaves that foreign buffer
current while ~literate-elisp-get-load-option~ falls back to the
=literate-load= property.  Asking a Markdown buffer for an Org
property is meaningless, and under Emacs 31 with Org 9.8 it is fatal:
Org runs its headline parser against the buffer's own
~outline-regexp~, derives a headline of level 0, and
~org-headline-re~ signals =rx ‘**’ range error=.

Reproducing it takes a real Markdown buffer with real content —
setting ~outline-regexp~ by hand is not enough, because Org's parser
only reaches the failing branch when the buffer's own outline
machinery matches something at point.  The test therefore skips where
~markdown-mode~ is absent rather than pretending a synthetic buffer
is equivalent.
#+BEGIN_SRC elisp :load test
(ert-deftest literate-elisp-load-from-foreign-buffer ()
  "Loading must not consult an unrelated current buffer for `literate-load'."
  (skip-unless (require 'markdown-mode nil t))
  (let ((file (make-temp-file "literate-elisp-foreign-" nil ".org")))
    (unwind-protect
        (progn
          (with-temp-file file
            (insert "* One\n\n#+BEGIN_SRC elisp\n"
                    "(defvar literate-elisp-test-variable-5 50)\n"
                    "#+END_SRC\n"))
          (makunbound 'literate-elisp-test-variable-5)
          (with-temp-buffer
            (markdown-mode)
            (insert "# heading\n\ntext\n")
            (goto-char 3)
            (literate-elisp-load file))
          (should (equal literate-elisp-test-variable-5 50)))
      (delete-file file))))
#+END_SRC
